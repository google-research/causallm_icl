...
from causallm_icl import self_attention_patch as fpatch
...
flags.DEFINE_bool("disable_softmax", default=False, help="Disable softmax.")
flags.DEFINE_bool("shared_block", default=False, help="Share TF block.")


def uniform_scaling(dtype: Dtype = jnp.float_):
...
def sinusoidal_init(max_len: int = 128) -> Callable[[Any, Any, Dtype], Array]:
  ...
  def init(key, shape: Tuple[int, ...], dtype: Dtype = np.float32) -> Array:
    """Sinusoidal init."""
    del key
    d_feature = shape[-1]
...
def nn_init_parser(init_str: str):
...
def nn_activation_parser(activation_str: str):
...
class TransformerConfig:
  ...
  activation_fn: Callable[..., Array] = functools.partial(
...
class PositionEmbeddings(nn.Module):
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    assert inputs.ndim == 3, "input dim != 3: %d" % inputs.ndim
...
class MLP(nn.Module):
  ...
  def __call__(self,
               inputs: Array,
               deterministic: Optional[bool] = True) -> Array:
...
class Encoder1DBlock(nn.Module):
  ...
  def __call__(
    ...
    y, attn_weights = fpatch.SelfAttention(
        ...
        disable_softmax=config.disable_softmax,
    )(x,
      mask=mask,
...
class Transformer(nn.Module):
  ...
  def __call__(self,
    ...
    if config.shared_block:  # use shared transformer block
      attn_block = Encoder1DBlock(config)
    for _ in range(config.num_layers):
      if not config.shared_block:  # not sharing transformer block
        attn_block = Encoder1DBlock(config)
...
def create_learning_rate_scheduler_v2(
    factors: str = "constant * linear_warmup",
