...
zeros = attention.initializers.zeros_init()
...
def dot_product_attention_weights(query: Array,
  ...
  Args:
    ...
    precision: numerical precision of the computation see `jax.lax.Precision`
      for details.
    disable_softmax: use linear attention instead of softmax attention.
  ...
  qk = jnp.einsum("...qhd,...khd->...hqk", query, key, precision=precision)
  qq = jnp.expand_dims(
      jnp.einsum("...qhd,...qhd->...hq", query, query, precision=precision),
      axis=-1)
  kk = jnp.expand_dims(
      jnp.einsum("...khd,...khd->...hk", key, key, precision=precision),
      axis=-2)
  attn_weights = qk - 0.5 * (qq + kk)  # RBF kernel
...
class MultiHeadDotProductAttention(Module):
  ...
  def __call__(
    ...
    x, attn_weights = self.attention_fn(
        ...
        disable_softmax=self.disable_softmax,
...
class SelfAttention(MultiHeadDotProductAttention):
  ...
  def __call__(
    ...
    Args:
      inputs_q: input queries of shape `[batch_sizes..., length, features]`.
      inputs_qv: not used for compatibility with the superclass.
      mask: attention mask of shape `[batch_sizes..., num_heads, query_length,
        key/value_length]`. Attention weights are masked out if their
        corresponding mask value is `False`.
      deterministic: if false, the attention weight is masked randomly using
